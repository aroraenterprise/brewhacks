...
class MerchantsResource(Resource):
    ...
    def post(self):
        ...
        merchants = MerchantModel.query().fetch()

        response = []
        products = ProductModel.query().fetch()

        for product in products:
            total_merchants = int(random.uniform(1, 4))
            random.shuffle(merchants)
            product.merchant = []
            for merchant in merchants[:total_merchants]:
                merchant.temp_key = merchant.key
                if not product.merchant:
                    product.merchant = []
                product.merchant.append(merchant)
            product.put()
            response.append(product.to_dict(include=ProductModel.get_public_properties()))
